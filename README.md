...
* [📥最新バージョン(r4)をダウンロード](https://github.com/hebiiro/whau/releases/download/r4/whau_r4.zip)
...
* `トークン/セグメントレイヤーオフセット` ✏️トークン/セグメント用オブジェクトを配置した場合に追加で空けるレイヤーの数を指定します。
* `開始/終了位置マージン` * ✏️セグメントのマージンを秒単位で指定します。`VADパディング`で制御できない場合に使用します。通常は0.0です。

## 📝PSDToolKit用オブジェクトの設定
