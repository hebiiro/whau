...
struct CommandPrompt
{
	...
	// コンストラクタです。
	//
	CommandPrompt(const std::wstring& dir)
	{
		// サブプロセスがハンドルを継承できるようにするための設定です。
...
#endif
		WCHAR command_line[MAX_PATH] = L"cmd.exe";
...
//		::WaitForSingleObject(pi.hProcess, INFINITE);
	...
	// コマンドを実行します。
	//
	BOOL execute_command(const std::string& command, BOOL wait = FALSE)
	{
		auto written = DWORD {};
		auto r1 = ::WriteFile(stdin_write, command.c_str(), (DWORD)command.size(), &written, nullptr);
		auto r2 = ::FlushFileBuffers(stdin_write);
		auto r3 = ::WaitForSingleObject(stdin_write, INFINITE);
		auto r4 = ::WaitForSingleObject(stdin_read, INFINITE);
		return r1;

		if (!r1) return r1;
		if (!wait) return r1;
		return ::FlushFileBuffers(stdin_write);
	}
};
